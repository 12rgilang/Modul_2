...
let Home = (props) => {
    ...
    let onTweet = async() =>{
        try {
            // STEP.1  ambil inputan dari text area
            let inputText = text .current.value

            // step.2 VAlidasi (maximum Char 160)
            if(inputText.length >= 160) throw { message: 'Max character is 160'}
            
            // Step.3 Simpan datanya
            let respon = await axios.post('http://localhost:5000/tweet', {tweet: inputText, userId: parseInt(localStorage.getItem('token'))})
            text.current.value = ''
            setError('')
            alert('tweet Scuccsess!')
            onGetData()
        } catch (error) {
